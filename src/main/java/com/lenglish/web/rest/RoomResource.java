...
import com.lenglish.service.RoomService;
...
public class RoomResource {
	 ...
	 *
	 * @param roomDTO
	 *            the roomDTO to create
	 * @return the ResponseEntity with status 201 (Created) and with body the new
	 *         roomDTO, or with status 400 (Bad Request) if the room has already an
	 *         ID
	...
	public ResponseEntity<RoomDTO> createRoom(@Valid @RequestBody RoomDTO roomDTO) throws URISyntaxException {
		...
		roomDTO.setCreateDate(DateTimeUtil.now());
	 ...
	 *
	 * @param roomDTO
	 *            the roomDTO to update
	 * @return the ResponseEntity with status 200 (OK) and with body the updated
	 *         roomDTO, or with status 400 (Bad Request) if the roomDTO is not
	 *         valid, or with status 500 (Internal Server Error) if the roomDTO
	 *         couldn't be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<RoomDTO> updateRoom(@Valid @RequestBody RoomDTO roomDTO) throws URISyntaxException {
		...
		RoomDTO result = roomService.save(roomDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, roomDTO.getId().toString()))
				.body(result);
	}
	 ...
	 *
	 * @param id
	 *            the id of the roomDTO to retrieve
	 * @return the ResponseEntity with status 200 (OK) and with body the roomDTO, or
	 ...
	 *
	 * @param id
	 *            the id of the roomDTO to delete
